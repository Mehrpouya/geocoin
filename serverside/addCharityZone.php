...
if ($db->connect_errno > 0) {
  ...
  $_POST['radius'],$_POST['zoneType'])){
    if ($db->connect_errno > 0) {
      die('Unable to connect to database [' . $db->connect_error . ']');
    } else {
      if (!($stmt = $db->prepare("INSERT INTO `point_zones`(`latitude`, `longitude`, `radius`, `zoneType`,`value`,`status`,`comment`) values (?,?,?,?,?,?,?)"))) {
      ...
      $stmt->bind_param('ddisdss', $latitude, $longitude , $radius,$zoneType,$value,$status,$comment);
      if (!$stmt->execute()) {
        $response = array(
          "status"=>"failed",
          "errno"=>$stmt->errno,
