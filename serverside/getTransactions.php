...
//if user passes long and lat and radius then we will return zones in this proximity otherwise return all zones.
  ...
  if ($db->connect_errno > 0) {
    ...
    // $sql = "SELECT t1.*,t2.zoneType,t2.walletId FROM `transactions` t1 join point_zones t2 where t1.zoneId=t2.id and  (t1.status='new' or t1.status='in_progress')";
    $sql = "SELECT * from `transactions` where `status`='new' or `status`='in_progress'";
