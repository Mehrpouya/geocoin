...
PointZoneClass.prototype.getZonesList=function(){
	var postUrl = "http://geocoin.eca.ed.ac.uk/getCharityZones.php";
	...
	$.post(postUrl,inputs, function( data ) {
		...
		self.getDroppedCoins();
		// console.log(data);
	});


};
var g_droppedMarkers=[];
PointZoneClass.prototype.getDroppedCoins=function(){
	var postUrl = "http://geocoin.eca.ed.ac.uk/getDroppedCoins.php";
	var inputs =  {};//for amsterdam add this so anywhere the user is we can detect the zones within x miles of user location.
	var self=this;
	$.post(postUrl,inputs, function( data ) {
		var zone;
		if(g_droppedMarkers.length>0)
		removeAllMarkers(g_droppedMarkers);
		for(var i = 0 ; i<data.length;i++){
			zone = data[i];
			myIcon = self.moneyIcon;
			if(zone.value<0)myIcon=self.antimoneyIcon;
			g_droppedMarkers[g_droppedMarkers.length] = L.marker([zone.latitude,zone.longitude], {icon: myIcon}).addTo(g_map);
			/*bindLabel("dropped by " + zone.userId, {
	    noHide: true,
	    direction: 'auto'
	}).*/
		}
...
PointZoneClass.prototype.loadZoneLists=function(_data){
	...
	console.log(this.continuesZones);
};
...
PointZoneClass.prototype.updateZonesMarkers=function(){
	...
	for(var i = 0 ; i<this.continuesZones.length ; i++){
		...
		g_continuesMarker[g_continuesMarker.length] = L.marker(newloc,{icon: myIcon}).bindLabel(zone.comment, {
    noHide: true,
    direction: 'auto'
}).addTo(g_map);
	}
	...
	for(var i = 0 ; i<this.continuesZones.length ; i++){
		...
		if(distanceToZone<zone.radius/1000){
			...
			$.post( postUrl,inputs).always(function(data){
				console.log(data);
				g_user.getNewBalabce();
				var url = "http://geocoin.eca.ed.ac.uk/addCoinDrop.php";
				var inputs =  {
