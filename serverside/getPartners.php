...
if(isset($_POST['uId'])){
  if ($db->connect_errno > 0) {
    ...
    if (!($stmt = $db->prepare("call getPartners (?)"))) {
    ...
    if (!$stmt->execute()) {
      $response = array(
        ...
        $stmt->bind_result($userName,$userId,$long,$lat);
        while ($stmt->fetch()) {
          $resultArray[]=array("userName"=>$userName, "userId"=>$userId,"longitude"=>$long,"latitude"=>$lat);
        }
        $response = array(
          "status"=>"sucess",
        "userIds"=>$resultArray
