...
*/
if(isset($_POST['userId'],$_POST['longitude'],$_POST['latitude'],$_POST['zoneid'])){
  if ($db->connect_errno > 0) {
    die('Unable to connect to database [' . $db->connect_error . ']');
  } else {
    if (!($stmt = $db->prepare("call marriageProposal (?,?,?,?)"))) {
    ...
    $zoneId = ($_POST['zoneid']);
    $stmt->bind_param('ddii', $latitude, $longitude, $userId,$zoneId);
    if (!$stmt->execute()) {
